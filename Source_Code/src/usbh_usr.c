...
//     LCD_LOG_Init();
	...
	Usart_Send("USB OTG HS HID Host \n\n",USARTPC);
#else
	Usart_Send("USB OTG FS HID Host \n",USARTPC);
#endif
	Usart_Send("USB Host library started \n",USARTPC);
  }
}
...
//   uint8_t temp[50];    
  ...
  sprintf(Buffer2,"\n VID: %0.4Xh ",(uint32_t)(*hs).idVendor);
  Usart_Send(Buffer2,USARTPC);
	
	sprintf(Buffer2,"\n PID: %0.4Xh ",(uint32_t)(*hs).idProduct);
  Usart_Send(Buffer2,USARTPC);
